...
export default function Loan() {
  ...
  const handleOnChange = async (e) => {
    ...
    if (e.target.name == "eth") {
      ...
      if (ethInputRef.current.value) {
        ...
        luckyInputRef.current.value = res.toNumber();
        setIsFetchingAmount(false);
      } else {
        luckyInputRef.current.value = "";
      }
    } else {
      // LUCKY
      if (luckyInputRef.current.value) {
        setIsFetchingAmount(true);
  ...
  const handleLoanProceed = async () => {
    if (ethInputRef.current.value < 1 || luckyInputRef.current.value < 0.0001) {
      ...
      if (res) {
        ...
        setLoanInformation({
          ...
          collateralAmount: luckyInputRef.current.value,
          duration: isActive,
          dateNow: date,
  ...
  return (
    <div className="mt-10 md:mt-24">
      ...
      <div className="w-auto mx-5 md:max-w-[450px] md:mx-auto ">
        <div className="text-[#3272e2] font-bold text-2xl mb-3">Borrow Request</div>
        <div className="bg-[#3272e2] p-4  shadow-xl  h-auto rounded-xl ">
          {" "}
          <div className="mt-3">
            <p className="my-1 mb-2 font-thin">
              How much do you want to borrow?
            </p>
            <div className="flex items-center w-full rounded h-16 outline-none bg-[#22437c] px-5  border-[1px] border-transparent hover:border-gray-600 transition  font-inter">
              <input
                name="eth"
                ref={ethInputRef}
          ...
          <div className="mt-5 z-[-1] flex items-center w-full rounded h-16 outline-none bg-[#22437c] px-5  border-[1px] border-transparent hover:border-gray-600 transition">
            <input
              name="LUCKY"
              ref={luckyInputRef}
              onKeyUp={(e) => handleKeyUp(e)}
              ...
              autoComplete="off"
            />
            <span className="text-2xl ml-5 cursor-auto select-none">wETH</span>
          </div>
          ...
          {!isValidLoanAmount && (
            <p className="font-light text-red-500 flex items-center select-none">
              ...
              Minumum Borrow amount is 1 FIL
            </p>
          )}
          {isFetchingAmont && (
            <div className="mt-4 flex items-center">
              <div className="w-5 h-5 border-t-2 border-yellow-300 rounded-full animate-spin "></div>
          ...
          {!account ? (
            <div
              ...
              className="mt-3 bg-[#22437c] px-2 py-2 md:py-3 rounded-2xl text-center text-white cursor-pointer hover:bg-[#1f5ba370] transition text-xl"
            >
              Connect Wallet
            </div>
          ) : !isProceeding ? (
